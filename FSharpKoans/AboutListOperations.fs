...
*)
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            ...
            innerPartition xs [] [] // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            ...
            innerInit [] 0 // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
        ...
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec innerTryFind xs=
                ...
                | head::tail -> match (p head) with
                                | true -> Some head
                                | _ -> innerTryFind tail
            innerTryFind xs  // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
        ...
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
                let rec innerTryFind xs=
                    ...
                    | head::tail -> let out = p head
                                    ...
                                    | _ -> out                        
                innerTryFind xs  // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
