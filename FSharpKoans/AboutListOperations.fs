    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec findLength list count=
                match list with
                | [] -> count
                | head::tail -> findLength tail count+1
            findLength xs 0 // write a function to find the length of a list
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec innerRev xs out=
                match xs with
                | [] -> out
                | head::tail -> innerRev tail (head::out)
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec innerMap xs out=
                match xs with
                | [] -> out
                | head::tail -> innerMap tail (out@[head+1])
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec innerMap xs out=
                match xs with
                | [] -> out
                | head::tail -> innerMap tail (out@[head*2])
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec innerMap xs out=
                match xs with
                | []-> out
                | head::tail -> innerMap tail (out@[f head])
        ...
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec innerFilter xs out=
                ...
                | head::tail -> match f (head) with
                                | true-> innerFilter tail (out@[head])
                                | _ -> innerFilter tail (out)
        ...
        let filter (xs : int list) : int list =
            let rec innerFilter xs out=
                ...
                | head::tail-> match head%2<>0 with
                               | true-> innerFilter tail (out@[head])
                               | _-> innerFilter tail (out) // write a function to filter for odd elements only.
            innerFilter xs []                           
...
*)
    ...
    let ``12 A fold which sums a list`` () =
        let fold initialState xs =
            let rec innerFold nextState xs=
                match xs with
                | []-> nextState
                | head::tail -> innerFold (head+nextState) (tail)  // write a function to do what's described above
    ...
    let ``13 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec innerFold nextState xs=
                match xs with 
                | [] -> nextState
                | head::tail -> innerFold (nextState*head) (tail)
            innerFold initialState xs // write a function to multiply the elements of a list
    ...
    let ``14 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec innerFold nextState xs=
                match xs with
                | [] -> nextState
                | head::tail -> innerFold (f nextState head) (tail)
            innerFold initialState xs  // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``16 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec innerExists xs=
                match xs with
                | [] -> false
                | head::tail -> match f head with
                                | true -> true
                                | _ -> innerExists tail
            innerExists xs // Does this: https://msdn.microsoft.com/visualfsharpdocs/conceptual/list.exists%5b%27t%5d-function-%5bfsharp%5d
    ...
    let ``17 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec innerPartition xs trueOut falseOut=
                match xs with
                | [] -> trueOut,falseOut
                | head::tail -> match (f head) with
                                | true -> innerPartition (tail) (trueOut@[head]) (falseOut)
                                | _ -> innerPartition (tail) (trueOut) (falseOut@[head])
            innerPartition xs [] [] // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.partition%5b't%5d-function-%5bfsharp%5d
    ...
    let ``18 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec innerInit out index=
                match index=n with
                | true -> out
                | _ -> innerInit (out@[f index]) (index+1)
            innerInit [] 0 // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.init%5b't%5d-function-%5bfsharp%5d
    ...
    let ``19 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec innerTryFind xs=
                match xs with
                | []-> None
                | head::tail -> match (p head) with
                                | true -> Some head
                                | _ -> innerTryFind tail
            innerTryFind xs  // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.tryfind%5b't%5d-function-%5bfsharp%5d
    ...
    let ``20 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
                let rec innerTryFind xs=
                    match xs with
                    | []-> None
                    | head::tail -> let out = p head
                                    match out with
                                    | None -> innerTryFind tail
                                    | _ -> out                        
                innerTryFind xs  // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.trypick%5b't,'u%5d-function-%5bfsharp%5d
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    *)
    ...
    let ``21 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec innerChoose xs out =
                ...
                | [] -> out
                | head::tail -> match p head with
                                | None -> innerChoose tail out
                                | Some item -> innerChoose tail (out@[item])                       
            innerChoose xs [] // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.choose%5b't,'u%5d-function-%5bfsharp%5d
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``22 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec innerMapi xs out index=
                match xs with
                | [] -> List.rev out
                | head::tail -> innerMapi tail ((f index head)::out) (index+1)
            innerMapi xs [] 0  // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.mapi%5b't,'u%5d-function-%5bfsharp%5d
